...
import com.example.projekta.adapter.ARejectAdmin;
...
public class FragmentReject extends Fragment implements DatePickerDialog.OnDateSetListener {
    View v;
    private Api_Rest apiRest;
    private RecyclerView recyclerView;
    private RecyclerView.LayoutManager layoutManager;
    private MaterialSpinner lstCustomer;
    private ARejectAdmin aRejectAdmin;
    ...
    public FragmentReject (Context context){
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        v = inflater.inflate(R.layout.reject_fragment,container,false);

        apiRest = Common.getApi();
        recyclerView = v.findViewById(R.id.rejectPesananAdmin_rv);
        lstCustomer = v.findViewById(R.id.cb_namaUserR);
        btn_tanggal = v.findViewById(R.id.btn_tanggalPandingR);
        btn_search = v.findViewById(R.id.btn_cariPandingR);
        btn_refresh = v.findViewById(R.id.btn_refreshPandingR);
        tv_tanggal = v.findViewById(R.id.tv_tanggalPandingR);
    ...
    private void loadListReject(){
        apiRest.listreject().enqueue(new Callback<List<MPandingAdmin>>() {
            @Override
            public void onResponse(Call<List<MPandingAdmin>> call, Response<List<MPandingAdmin>> response) {
                if (response.body()!=null){
                    ...
                    aRejectAdmin = new ARejectAdmin(v.getContext(),mPandingAdmins);
                    recyclerView.setAdapter(aRejectAdmin);
                }else {
                    Toasty.info(context,"Tidak Ada Pesanan Di Tolak",Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<MPandingAdmin>> call, Throwable t) {
                Toasty.error(v.getContext(), "Tidak Ada Pesanan Di Tolak", Toast.LENGTH_SHORT).show();
            }
        });
    }
        ...
        apiRest.searchlistpandingadmin(stanggal,scustomer_id,status).enqueue(new Callback<List<MPandingAdmin>>() {
            ...
            public void onResponse(Call<List<MPandingAdmin>> call, Response<List<MPandingAdmin>> response) {
                if (response.body()!=null){
                    List<MPandingAdmin> mPandingAdmins = response.body();
                    aRejectAdmin = new ARejectAdmin(v.getContext(),mPandingAdmins);
                    aRejectAdmin.notifyDataSetChanged();
                    recyclerView.setAdapter(aRejectAdmin);
                }else {
