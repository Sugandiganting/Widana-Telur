...
import com.example.projekta.adapter.ACreditKurir;
...
public class FragmentCreditKurir extends Fragment implements DatePickerDialog.OnDateSetListener {
    ...
    private ACreditKurir aPandingAdmin;
    ...
    private String id_kurir,type_payment="1",status="3";
    User user = new User();

    public FragmentCreditKurir(Context context){
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        v = inflater.inflate(R.layout.credit_kurir_fragment, container, false);

        apiRest = Common.getApi();
        user = User.findById(User.class, (long) 1);
        id_kurir = user.getId_user();
        recyclerView = v.findViewById(R.id.creditKurir_rv);
        lstCustomer = v.findViewById(R.id.cb_namaUserKK);
        btn_tanggal = v.findViewById(R.id.btn_tanggalPengirimanKK);
        btn_search = v.findViewById(R.id.btn_cariKK);
        btn_refresh = v.findViewById(R.id.btn_refreshKK);
        tv_tanggal = v.findViewById(R.id.tv_tanggalPengirimanKK);
    ...
    private void loadListCreditKurir() {
        String sid_kurir = id_kurir;
        apiRest.listcreditkurir(sid_kurir).enqueue(new Callback<List<MPandingAdmin>>() {
            @Override
            public void onResponse(Call<List<MPandingAdmin>> call, Response<List<MPandingAdmin>> response) {
                if (response.body()!=null){
                    ...
                    aPandingAdmin = new ACreditKurir(v.getContext(), mPandingAdmins);
